...
Craze Seducing Our Daughters." Shrier's thesis, which becomes more and
more explicit as the book progresses, is that minors, who do not know
...
our current sociopolitical moment. 

Whereas Sedgwick offers reparative reading that responds to the
practices of paranoia, I pose it as a kind of reading that responds
specifically to the condition of /text capture/ by automated systems.

The end of the chapter demonstrates with an example for analyzing
political polarization today around debates on gender rights.
...
*** TO_DRAFT close reading: aclu & heritage
- reparative reading is about close attention to language
  - finding unexpected meanings in this language.
  - ultimately, re-working capture takes language that has been used
    to oppress, and instead finding nourishment.

**** web crawling: aclu & heritage.org
The first dataset I scraped from the ACLU's webpage on "transgender
rights" ("Transgender Rights"). (This process of re-training is
...
I trained a pair of Large Language Models based on highly polarized
sources. To train these models, I used a "base" model, called GPT-2,
an early, open-source iteration of the GPT model series from /OpenAI/
(Radford et al.) I then re-trained the base model on new, custom
datasets, one based on a progressive news source, the American Civil
Liberties Union (ACLU), and another on a conservative news source, the
Heritage Foundation. 
...
Chapter Five uses text generation as an analytical tool for surfacing
the intersection of perspectives around gender. It does this because
machine learning, as I explain in the chapter, aggregates perspectives
