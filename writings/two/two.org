...
After cleaning, the first setnence would appear as the following list
of words:
...
textual patterns, requires transforming the source text into
computable units. Cleaning the text strips capitalized words,
...
to find connections among more apparently substantial terms.

4

However, the word "He," and the em dash following and accentuating
that "He," is hardly minor in a text that interrogates gender
differences and whose protagonist experiences their own shift in
gender. Indeed, as Pamela Caughie puts it, the pronoun and punctuation
in the first sentence effectively announces the book's investment in
subverting gender norms: "immediately our doubt is aroused... The
emphasis on an innocent pronoun makes it suspect" ("Double Discourse"
42).

5

This chapter examines how quantitative text analysis might be used to
analyze gender, using Woolf’s /Orlando/ as a test case. It explores an
experimental approach to text analysis that draws from computer
programming and gender theory, Judith Butler's theory of gender
performativity. This analysis surfaces a mechanism, specifically a
technical constraint, common to both text analysis and gender
performativity, which is /iteration/. I demonstrate that the logic of
repetition that drives iteration can be re-worked into a new text
analysis procedure that brings previously suppressed details about
gender, and the gender binary, to the surface. The final section of
the chapter thus illustrates a text analysis method that re-signifies
the meaning of binary gender terms, of "man" and "woman." It works
within the constraining structure of iteration in order to destabilize
the binary from the inside.

** Falsifiable
1

The history of text analysis as a practice has been traced to many
origins.[fn:1] My particular version begins with Franco Moretti, whose
...
Shortly after the publication of a /New York Times/ profile of the
...
published a collection of essays, /Signs Taken For Wonders: On the
...
"reproducible."[fn:2] Reproducible methodologies obviate the risk of
...
quantitative practices. Rather than close attention to individual
samples, quantification counts patterns of specific features across
wide swaths of text. As we will see below, it simplifies text into
workable units, which is the basis for higher levels of computation.
These simple and workable units, at large scales, enable ambitious
interpreptations. As Moretti explains, "If we want to understand the
system in its entirety, we must accept losing something. We always pay
a price for theoretical knowledge" (/Distant Reading/ 49).

5

As a graduate student, I found Moretti's writing both striking and
alluring. He is an assertive writer, using mostly short sentences that
give his prose a plucky rhythm. Moreover, despite drawing from
...
is striking. It showcases Moretti's characteristic way of transforming
inferences, premises, and hypotheses into declarative statements,
stating a claim or hypothesis as an objective or self-evident fact. It
suppresses the possibility of alternative views---falsifying them
before they have the chance to be heard.[fn:3]
...
"make a little attempt." As Stephen Ramsay points out, "data is
presented to us... not as something that is also in need of
interpretation" (Ramsay, /Reading Machines/ 5). Proffering his
insights as a description of reality, Moretti misrepresents the role
that inferential thinking plays in his analysis, which contain
provocative conclusions, for example, that titles function as a proxy
for market forces, as "a code, in the market... where the novel as
language meets the novel as commodity" (/Distant Reading/ 181).

8

Crucially, as Lauren Klein points out, distant reading can overlook
details which are necessary to cultural critique. In a talk titled
"Distant Reading After Moretti," Klein explains,
#+begin_quote
...
slavery today" (665). Her work demonstrates that sometimes, entire
lives are deeply deeply distributed text, and require close and
careful reading. 

10

Indeed, the faith in the falsifiable may be a proxy for avoiding
certain kinds of cultural critique. More than avoidant, I want to
argue, fasifiable criticism is intellectually coercive. It functions
through an amplification and valorization of what is seemingly
objective---in particular, the counting of words. With word counts and
...
with close reading. As Moretti says, "we know how to read texts, now
let's learn how /not/ to read them" (/Distant Reading/ 48).

But more than that, it can also be a way of silencing less prominent
or otherwise inconvenient views. As an imposition of a view from the
top, it functions like other hierarchies of power, like academic
prestige. Like when an older, accomplished male professor gets away
with telling a young, female graduate student that "You American girls
...
models" of literary data, where models represent different datasets
associated with different "perspectives." In one project, Underwood
uses a binary classification algorithm to cluster terms that male and
female writers tend to associate with either gender.

His results show that both male and female writers tend to agree on
terms that describe the male gender, like "pocket," "beard," and
"grinned," and the female gender, like "hair," "eyes," and "mother".
When writers describe characters of their own gender, however, there
is disagreement: writers tend to associate their own gender with terms
like "wondered," "mind," and "head," and the opposite gender with
terms like "meant," "kissed," "tone." The results, which Underwood
presents in a visual chart showing two diagonals crossed in
opposition, dramatizes a perspectival shift between the two genders.

12

In this example, the reinscription of the "binary structure" happens
not only in the choice of model, a classification (i.e. binary) model
which makes predictions on a yes/no or, in this case, a male/femal
scale. It also happens with the choice in visualizing the results in a
way that places male and female genders in opposition. The diagonal
reinforce a stereotypical active/passive binary (among other binaries)
between men and women and, although based on agreement between the
genders and their relationships to themselves, nonetheless imposes an
/opposition/ between how male and female authors describe each other.

When explaining the diagram, Underwood himself admits that "gender
...
Learning and Human Perspective” 98). Beyond this reification of the
gender binary, however, Underwood's visualization effectively deploys
the binary /as a model/ for proving the value of quantitative methods.
In order for his experiment to take place, the gender binary is a
necessary presumption, a "ground truth," that makes the choice of a
classification model appear appropriate. In other words, before he
even constructs the diagram, gender roles have been already
manufactured into opposition.

14

Posing one gender against another is not, I believe, as provocative as
work that explicitly deconstruct these categories (and categorization)
in the first place. For example, Laura Mandell, who uses stylometry to
study sentimental and anti-Jacobin novels, creates new categories that
draw gender into genre. She creates doubly refracted categories like,
"men writing as men," "women writing as women," "women writing as
men," "men writing as women."[fn:4]
...
variance of Black authorship. In other words, without Black
authorship, White authorship would cease to be visible as a coherent
category.
...
single word--"appalled"--whose presence, according to the model, has
...
their analysis to connect gender to race: “the moment David develops a
troubled relationship to normative masculinity [as] also the moment he
becomes ‘White’” (71).

16

Like Underwood, So and Roland began with the binary form, the binary
categories of Black/White. But unlike Underwood, they ended with its
fracturing. This fracturing, evident in the computer's
misclassification of authorship in /Giovanni's Room/, leads to a
reading which coordinates race with gender and sexuality. This
"queering the machine’s color line," the authors explain, opens up new
readings of intersectionality in the novel which "in their current
form, our data and model are not robust enough to handle" (72). Rather
than reproducible, this method is fracturing and refractive, both
reflecting and corrupting the constructed nature of binary structures
back at the critic. It brings us closer to the methodology that I
outline in the next section, /iteration/.

** Iteration in gender
2

I now turn to Judith Butler's theory of gender performativity,[fn:5]
and to a particular mechanism that drives gender performativity:
/iteration/. As I explain below, iteration is an essential and
constraining mechanism that binds performativity to its effects.
However, as Butler explains, this constraint can be exploited to
subvert the norm. 

My focus on iteration addresses a common misreading of performativity,
that it denotes a series of acts that can be imitated at will. Indeed,
Butler's book, /Bodies that Matter: on the Discursive Limits of Sex/
(1996), which follows /Gender Trouble/, explicitly aims to clarify how
performativity is compulsory, bounded on all sides by social norms in
ways that delimit a subject's agency. In /Bodies that Matter/, Butler
explains that gender performativity is a pre-determined procedure that
/constitutes/ subjectivity: "there is no subject who decides on its
gender.... on the contrary, gender is part of what decides the
subject" (ix).

In Butler's theory, iteration is a /perpetual re-creation of norms/,
which effectively /produces and re-produces power/. As Butler
explains, "performativity must be understood not as a singular or
deliberate 'act,' but, rather, as the reiterative and citational
practice by which discourse produces the effects that it names" (xii).

For example, according to Butler, it is through performativity that
"sexed" bodies emerge. They explain that "'Sex' is not a simple fact
...
biological category is actually a discursive "sedimentation," formed
through continual repetition.

Despite this compulsive repetition, performativity can be subverted,
according to Butler. Subversion is possible through manipulating the
iterative process. This manipulation brings something something
unexpected and extraneous into performativiy, while nonetheless
staying within the boundary of the permissable.


To theorize what is permissable and what is not, Butler draws from
poststructuralist feminist theorist Luce Irigaray's theory of the
internal vs the external feminine. Taking philosophical discourse as
an example, Irigaray argues that philosophers, like Plato, Aristotle,
Descartes, among others, represent the gender binary "on the basis of
masculine parameters" (18). Irigaray argues that the binary,
constructed along masculine terms, precludes the representation of the
the feminine, which is effectively excluded from the binary system.
The feminine that exists within the system, the internal feminine, is
a "domesticated" feminine.

Although this external feminine is unrepresentable, it is the /matter/
from which the Male/Female binary comes into intelligibility: "The
economy that claims to include the feminine as the subordinate term in
a binary opposition of masculine/feminine excludes the feminine,
produces the feminine as that which must be excluded for that economy
to operate" (10). As it happens, Butler refers to the excluded
feminine as a /matrix/. With its etymological associations to
"mother," "mater," and the "womb," the matrix is a generative force,
"an inscriptional space that... designates the constitutive outside of
the Platonic economy; it is what must be excluded for that economy to
posture as internally coherent" (12-13).

It is not possible, according to Butler (following Irigary), to invoke
the excluded feminine outside of masculine parameters. However,
subversion is possible by breaking open performativity processes,
specifically, the process of iteration. For performativity, whereby
meaning is signified and re-signified endlessly, is never complete.
...
to identity. Gender identity must be, through iteration, stabilized
repeatedly throguh a field of potential exclusions.

8

Ant it is the material of the excluded which can come back, Butler
says, as a "troubling return" (xxx). As Butler explains, every
"identification takes place through a repudiation which produces a
domain of abjection" (xiii). These repudiated, disavowed identities
offer "a field of disruptive possibillities," where terms may draw
from rejected meanings to be resignified. Such resignfications
...
economy" (Butler 18). In an interesting double inhabitation, where
Butler mimicks Irigaray mimicking Plato, Butler writes,
#+begin_quote
...
This iteration between the internal and external (repudiated) feminine
is another place where Butler's theory is often misunderstood. For
example, one text analysis project, cited above, argues that
"subversion" may occur through straight characters that employ
queer-coded language. Analyzing the transcripts of a popular TV show,
/The Big Bang Theory/, the authors claim that "an undercurrent of
emotional intimacy and dependency... challenges the rigid boundaries
of what is considered acceptable" (Chang et al., 10). However, the
characters in question remain within heterosexual relationships, with
their admittedly romantic language being relegated to punchlines. Like
some comedies which feature crossdressing (/Some Like It Hot/,
/Tootsie/), which Butler describes as "drag as high het
entertainment," this show "produce[s] and contain[s] the homosexual
excess" in a way that ends up propping up the heterosexual regime
(85). Rather than the excluded or repudiated queerness, it is closer
to what Irigaray describes as a "domesticated" queerness.

12

Subversion then, works toward displacement through occupation and
resemblance. One way that it occurs is through iteration, as Irigaray
does with her mimicry of Plato's style. Through the iterative process,
one brings back again and again the repudiated meaning: through
subservience, insubordination; and through resemblance, deception.
...
To prepare a text for analysis, one uses iteration to process the
text, one word at at time. Typically this involves "cleaning" textual
elements that are undesirable for the purpose of text analysis, like
like articles and prepositions whose high frequencies would obstruct
word counts.[fn:6] Using a loop, a program /iterates/ through each
word in a text, checks whether it needs to be "cleaned," and then
perform a set of cleaning actions.
...
The loop will take the first word, "He," and then the second word,
which is in this case a punctuation, an em dash, "---," and so on. The
third line contains a filter, ~isalpha()~, which checks if the word is
comprised of only alphabetic characters (i.e. it is not a number or
punctuation). If the word is alphabetic, it passes the filter (at this
point, the em dash would fail this requirement, and wouldn't pass to
the next line). At the next line, the word is lowercased, ~lower()~
and finally added to the empty list, ~clean~.
...
alphabetic and lowercased letters.

Below is the first sentence of the novel with all words in are
...
This step can be added as an additional filter to the above loop.
After passing the filter that checks for alphabetic text, the loop
will then check for stop words. If the word is /not/ a stop word, it
will pass to the next line to be lowercased and added to the list
~clean~.
...
The following words now remain from the first sentence of Orlando:
...
The final step of cleaning is to get each word's root form, the
"lemma," so as to avoid counting the same word as instances of
different words, like "run" and "running," or "child" and "children."

To do so, we add a line to our loop that look up each word, one by
one, and returns the shortest lemma found.[fn:7] In the python code,
...
This function takes a canonical concept from computational
linguistics, called "distributional similarity," which measures how
often terms appear in samples of language data, and assumes that words
which appear in similar contexts tend to have similar meanings.
...
similarity, which is computed by counting words and their neighbors.[fn:8]

13

In fact, the simple act of counting words is also how /deep learning/
...
“woman” to other words in the dataset.[fn:9] 
...
with statistics, linear algebra, and calculus. For example, cosine
...
Like Butler's matter or /matrix/, here is another matrix that produces
meaning. And this matrix similarly operates according to masculinist
parameters. Gender is imbricated with conceptions of power ("King").
Furthermore, gender becomes an operand for computing language meaning.
Much like Ted Underwood's graph which only understands gender when it
is placed into an oppositional relationship, gender here is the
/model/ which shows the algorithm to be true.

16

The goal, for the following text analysis, will not be to re-draw the
matrix. Rather, it will be, in Butler's words, "to show that it is
occupiable" (11). In the next section, I use iteration as a
methodology that re-works text analysis procedures into a practice
that deconstructs the terms of the gender binary. The goal will not be
to dispel or revise the terms of the binary, but to show that these
terms are /already/ inhabited by that which they seek to exclude.

** Fluidity
1

Subtitled /A Biography/, the novel /Orlando/ takes real life as its
source material. First, English literary history is reproduced and
satirized, with each chapter of the novel taking a different period,
like the Elizabethan, Restoration, and Victorian periods, and
featuring historical figures like Queen Elizabeth and Alexander Pope
as characters.[fn:10] Orlando's character herself is modeled on Woolf's
...
original context. In doing so, it iterates through various gender
terms, starting with the terms from the gender binary, "man" and
"woman."

This toggling between close and distant views of language in the novel
...
significant" (17).[fn:12] Unlike Piper, however, who uses the
...
inclination as the driving force of analysis (17). It is closer, in
reality, to approaches described by both Tanya Clement and Katherine
Bode, who view the act of computation as necessarily guided by the
critic's own hand.[fn:13] Bode, who draws from feminist physicist
Karen Barad's concept of "intra-action," where objects and observers
come into being through acts of observation, suggests that "we can
understand our databases or models as part of the ongoing
materialisation of literary texts as /emerging events/ always arising
from an altering how the literary past as reconfigured" (emphasis
mine).[fn:14] Clement, whose multimodal work blends textual, audio,
and visual registers, describes the critic as an "observer
...
specific hermeneutical framework."[fn:15]
...
function, ~most_similar()~, that applies cosine similarity to
calculate the words "nearest" to the target words.[fn:16]
...
This analysis begins with an oppositional formulation of gender, one
that places male and female into a positive/negative axis---in fact,
much like Underwood's classification model. However, the goal here is
to bring forth what has been /repudiated/ by each term in the binary.
Following Butler's reading of Irigaray reading Plato, my goal is to
show that what is excluded from the binary is nonetheless present in
the matrix which generates that binary---here, within a literal matrix
of word vectors. In what follows, then, I read into a selection of
these terms, examining them in context, to seek out what constitutes
their meaning and, eventually, what must be repudiated in order for
them to signify.
...
withholding and, eventually, yielding. Yet, despite this passivity, it
not totally submissive: it implies a kind of power, the power of
refusal.
...
This term occurs in the same scene as above. Its appearance, however,
is more conspicuous than the former: it emerges in a sentence where
Orlando is considering her sexuality explicitly---indeed, one of the
very few references in the novel to sexuality.
#+begin_quote
...
sunlight, into "the yellow pools which /chequered/ the floor" (12).
This depiction luxuriates over Orlando's face and body:
#+begin_quote
...
the sun itself. (12)
#+end_quote
This image of Orlando's body /chequered/ into different colored pieces
...
As a term, "despair" crosses the lines of the binary. It comes from
the "feminine" list, but unlike most of the other terms on the list,
it describes a moment in the narrative when Orlando is still a man.
In other words, like Orlando, this term has gender-crossing
capabilities. 

Indeed, throughout the novel, language and gender appear to be
co-constructed. An interesting passage, which contains words from both
the woman and man lists, shows how language's failure to describe
reality is related to how gender expreses itself.

In this passage, Orlando beholds Sasha for the first time, and he
cannot tell whether she is a man or a woman. In his tumultuous and
agonizing attempt to determine her gender, the words “curiosity,” from
the “man” list of similar words, and “seductiveness," from the "woman"
list, both appear.
...
qualities. Gender, like language, is /fluid/.
...
embodiment as /telos/" (33).

And particularly, the novel /Orlando/, according to Prosser, bears out
...
sexed body at all but the cultural vicissitudes of gender... Orlando
...
/Orlando/ will have noticed that, in the introduction of the chapter,
...
Like /Orlando/'s conspicuous narrator, there is something conspicuous
...
A decade after Bulter developed her concept of Gender Performativity,
Omise’eke Natasha Tinsley writes about the problem of gender fluidity
as a metaphor. Her critique re-works the trope of fluidity which,
drawing from the ocean, “is not an easy metaphor or queer and racially
hybrid identities but for concrete, painful, and liberatory
experience” (192-193). For Tinsley, fluidity is an opportunity for “a
return to the materiality of water to make its metaphors mean more
complexly, shaking off settling into frozen figures” (212). Tinsley
theorizes fluidity as being stripped by the water, particulars of
...
[fn:1] One rendition traces this lineage to Roberto Busa, an Italian
Jesuit priest who authored the first digital condordance, the /Index
Thomisticus/ (See Ramsay, Stephen, /Writing Machines/. 2011). Another
traces this practice to the intersection of literary studies and
social science, centering in particular the work of marxist scholar
Raymond Williams and feminist scholar Janice Radway (See Ted
Underwood, "A Genaeology of Distant Reading").
...
[fn:3] The faith in the falsifiable, in the power of supposedly
...
champion them: she /agrees/ about the objectivity of their procedures.
...
[fn:4] Mandell, Laura. "Gender and Cultural Analytics:: Finding or
Making Stereotypes?" par. 35.

[fn:5] Some text analysis projects take the language of performance
...
contexts," including computational tools. From the field of
...
in TV show dialogue, to predict whether samples of a TV show, /The Big
Bang Theory/, are queer-coded. Bode, Katherine. “Data beyond
representation: From computational modelling to performative
materiality.” MLA Convention, Seattle, 10 January 2020.
https://katherinebode.wordpress.com/home/mla-convention-2020/

[fn:6] Elements that are highly frequent, like articles, prepositions,
and even pronouns, can drown out other terms which are considered more
substantial, like nouns and verbs. Similarly, irregular cases are
standardized, because capital letters would count toward different
words (for example, "talk" and "Talk" would be considered two
different words). For similar reasons, word endings and inflections
like plural forms (endings in "s") and tenses ('walked', 'walks' or
'walking') are converted into a root form.

[fn:7]  It uses the WordNet database, an electronic lexical database
...
[fn:8] Importantly, the computer does not understand or impute
/meaning/ to the words. Rather, it only counts each word as a string,
that is, as a piece of data composed of alphanumeric sequences.

[fn:9] Known as the "GLoVe" dataset: Jeffrey Pennington, Richard
Socher, and Christopher D. Manning. 2014. GloVe: Global Vectors for
Word Representation.

[fn:10] According to Christy L. Burns and Jane De Gay, Woolf re-works
...
in Virginia Woolf's Orlando.” *Twentieth Century Literature*, vol. 40,
no. 3, 1994, pp. 342–364. *JSTOR*, www.jstor.org/stable/441560; De
Gay, Jane. "Virginia Woolf's feminist historiography in Orlando."
Critical Survey, vol. 19, no. 1, 2007, p. 62+.

[fn:11] “‘If I Saw You Would You Kiss Me?": Sapphism and the
Subversiveness of Virginia Woolf's Orlando.” /PMLA/, vol. 103, no. 1,
1988, pp. 24–34. www.jstor.org/stable/462459

[fn:12] Andrew Piper, /Enumerations/, University of Chicago
Press, 2019. page 18.

[fn:13] Karen Barad, in fact, takes up the concept of performativity
...
[fn:14] Bode, Katherine. "Computational Modeling: From Data
Representation to Performative Materiality." ATNU/IES Virtual Speaker
Series 2020/2021. Newcastle University, UK. November 26, 2020.

[fn:15] Clement, T. (2013). Distant Listening or Playing
Visualisations Pleasantly with the Eyes and Ears. /Digital Studies/le
Champ Numérique/, 3(2). DOI: http://doi.org/10.16995/dscn.236

[fn:16]  The documentation describes ~similar_words()~ "computes
...
[fn:17] Link to file from this project's source code:
https://github.com/gofilipa/qdr/blob/master/lib/actions.py

[fn:18] Criticism of the novel about gender and story.

[fn:19] link to source Counter (python) code:
...
[fn:20] De Gay, Jane. “Virginia Woolf’s Feminist Historiography In
Orlando.” /Critical Survey/, vol. 19, no. 1, 2007, p. 62+.

[fn:21] "'Ransacking the Language': Finding the Missing Goods in
Virginia Woolf's Orlando." Journal of Modern Literature, vol. 29 no.
4, 2006, pp. 57-75. Project MUSE, doi:10.1353/jml.2006.0050

[fn:22] Channing, Jill. "Magical realism and gender variability in
Orlando." Virginia Woolf Miscellany, no. 67, 2005, p. 11+.

[fn:23] Jessica Berman, "Is the Trans in Transnational the Trans in
Transgender?" /Modernsim/Modernity/, vol 24, no 2, 2017. 217-244.

[fn:24] Pamela Caughie. “Virginia Woolf’s Double Discourse.”
...
[fn:25] Distant reading is permeated by great ambition. In
/Macroanalysis: Digital Methods and Literary History/ (2013), Matthew
Jockers "ask[s] very big questions... regarding how literary trends
are employed over time, across periods, within regions"; In /Distant
