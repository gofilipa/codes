...
\label{sec:org16ea032}

\subsection{Doubt}
\label{sec:org047376b}
1
...
environments.\footnote{In fact, Barad takes up the concept of performativity which
...
Performativity," 2012, footnote 27 on page 49.} Bode suggests that "we can understand our
databases or models as part of the ongoing materialisation of literary
texts as \emph{emerging events} always arising from an altering how the
...
other work uses the binary to \emph{deconstruct} categorization in the
...
\subsection{Fluidity}
\label{sec:orgf4942b2}
For Butler, following Irigaray, the matrix cannot express the feminine
on its own terms. Rather, what it expresses is a "domesticated"
feminine, the feminine according to masculine parameters. In order to
bring the external feminine into inscription, one relies on strategy
of iteration, where each instance of reiteration brings something
unexpected or excluded to trouble the stability of terms. In what
follows, I pursue an iterative reading practice of gender that relies
on iterating through various gender terms, not to get at the truth of
the gender binary (whatever that may be), but to show that it is
already inhabited by that which it seeks to exclude.
...
\emph{matters}) through this compulsory repetition of gender norms.
