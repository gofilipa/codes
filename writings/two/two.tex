...
\label{sec:org9921c5f}

\subsection{Doubt}
\label{sec:orgb75dff6}
1
...
subverting gender norms: "immediately our doubt is aroused\ldots{} The
...
analyze gender, using Woolf’s \emph{Orlando} as a test case. It explores an
...
performativity, which is \emph{iteration}. I demonstrate that the logic of
...
Underwood, "A Genaeology of Distant Reading").} My particular version begins with Franco Moretti, whose
...
a price for theoretical knowledge" (\emph{Distant Reading} 49).
...
before they have the chance to be heard.\footnote{The faith in the falsifiable, in the power of supposedly
...
interpretation" (Ramsay, \emph{Reading Machines} 5). Proffering his
...
for market forces, as "a code, in the market\ldots{} where the novel as
language meets the novel as commodity" (\emph{Distant Reading} 181).
...
let's learn how \emph{not} to read them" (\emph{Distant Reading} 48).
...
\emph{opposition} between how male and female authors describe each other.
...
the binary \emph{as a model} for proving the value of quantitative methods.
...
men," "men writing as women."\footnote{Mandell, Laura. "Gender and Cultural Analytics:: Finding or
...
misclassification of authorship in \emph{Giovanni's Room}, leads to a
...
outline in the next section, \emph{iteration}.

\subsection{Iteration in gender}
\label{sec:orgfe9ba13}
2

I now turn to Judith Butler's theory of gender performativity,\footnote{Some text analysis projects take the language of performance
...
\emph{iteration}. As I explain below, iteration is an essential and
...
Butler's book, \emph{Bodies that Matter: on the Discursive Limits of Sex}
(1996), which follows \emph{Gender Trouble}, explicitly aims to clarify how
performativity is compulsory, bounded on all sides by social norms in
ways that delimit a subject's agency. In \emph{Bodies that Matter}, Butler
explains that gender performativity is a pre-determined procedure that
\emph{constitutes} subjectivity: "there is no subject who decides on its
gender\ldots{}. on the contrary, gender is part of what decides the
subject" (ix).

In Butler's theory, iteration is a \emph{perpetual re-creation of norms},
which effectively \emph{produces and re-produces power}. As Butler
...
Although this external feminine is unrepresentable, it is the \emph{matter}
...
feminine as a \emph{matrix}. With its etymological associations to
"mother," "mater," and the "womb," the matrix is a generative force,
"an inscriptional space that\ldots{} designates the constitutive outside of
...
\emph{The Big Bang Theory}, the authors claim that "an undercurrent of
emotional intimacy and dependency\ldots{} challenges the rigid boundaries
...
some comedies which feature crossdressing (\emph{Some Like It Hot},
\emph{Tootsie}), which Butler describes as "drag as high het
...
word counts.\footnote{Elements that are highly frequent, like articles, prepositions,
...
'walking') are converted into a root form.} Using a loop, a program \emph{iterates} through each
...
third line contains a filter, \texttt{isalpha()}, which checks if the word is
...
the next line). At the next line, the word is lowercased, \texttt{lower()}
and finally added to the empty list, \texttt{clean}.
...
will then check for stop words. If the word is \emph{not} a stop word, it
...
similarity, which is computed by counting words and their neighbors.\footnote{Importantly, the computer does not understand or impute
\emph{meaning} to the words. Rather, it only counts each word as a string,
...
\emph{model} which shows the algorithm to be true.
...
terms are \emph{already} inhabited by that which they seek to exclude.

\subsection{Fluidity}
\label{sec:org0f7cf2e}
1

Subtitled \emph{A Biography}, the novel \emph{Orlando} takes real life as its
...
as characters.\footnote{According to Christy L. Burns and Jane De Gay, Woolf re-works
...
critic's own hand.\footnote{Karen Barad, in fact, takes up the concept of performativity
...
Performativity," 2012, footnote 27 on page 49.} Bode, who draws from feminist physicist
...
materialisation of literary texts as \emph{emerging events} always arising
from an altering how the literary past as reconfigured" (emphasis
mine).\footnote{Bode, Katherine. "Computational Modeling: From Data
Representation to Performative Materiality." ATNU/IES Virtual Speaker
Series 2020/2021. Newcastle University, UK. November 26, 2020.} Clement, whose multimodal work blends textual, audio,
...
to bring forth what has been \emph{repudiated} by each term in the binary.
