...
\label{sec:org337a5ac}

\subsection{Doubt}
\label{sec:orgbd6fac5}
1
...
change. \emph{Orlando: A Biography} is a fantastical novel that defies
...
gender by using Woolf’s \emph{Orlando} as a test case. It explores an
...
analysis and gender performativity, which is \emph{iteration}. There is an
...
various gender terms in \emph{Orlando}, to re-signify their meaning within
...
\label{sec:org9c551c6}
1

Text analysis is often driven by great ambitions. In \emph{Macroanalysis:
Digital Methods and Literary History} (2013), Matthew Jockers "ask[s]
very big questions\ldots{} regarding how literary trends are employed over
time, across periods, within regions"; In \emph{Distant Horizons: Digital
...
methods, \emph{Language Machines: Cultural AI and the End of Remainder
...
In the wake of a 2017 profile by \emph{New York Times}, three students
...
reproducible methods.\footnote{Sarah Cohen-Boulakia, Khalid Belhajjame, Olivier Collin, Jérôme
...
\url{https://doi.org/10.1016/j.future.2017.01.012}.} In those disciplines, reproducibility
means that the analytical procedure will yield the same result.\footnote{Reproducibility and transparency: what's going on and how can
...
learn how \emph{not} to read them" (\emph{Distant Reading} 48). Quantification,
...
of their overall interconnection” (\emph{Graphs, Maps, Trees} 1). These
...
something. We always pay a price for theoretical knowledge" (\emph{Distant
...
nouns and adjectives. But they do not, on their own, \emph{learn to
compress meaning} or \emph{develop special 'signals'}. As Stephen Ramsay
points out, Moretti presents his own insights as a description of
reality, as if "data is presented to us\ldots{} not as something that is
also in need of interpretation" (Ramsay, \emph{Reading Machines} 5).
...
stories.\footnote{In one project, Klein uses network analysis methods to bring
...
\emph{close down} close reading and the pesky critiques asssociated with
it. One might finally approach a \emph{falsifiable criticism}, to be able
...
physical body, Butler explains, language must first \emph{assume} a body.
...
realization: "the mimetic or representational status of language\ldots{} is
not mimetic at all. On the contrary, it is productive, constitutive,
one might even argue performative" (\emph{Bodies} 6).
...
expectations" (4). The authors argue that in the TV show \emph{Big Bang
...
functions for simple text analysis tasks, like q \texttt{concordance()},
which displays the context window surrounding a given word, and
\texttt{similar()}, which finds words that are used in similar contexts to a
given word.
...
gender into an oppositional relationship, gender is the \emph{model} which
...
Orlando is as unstable as the sex of Orlando" (42). Subtitled \emph{A
Biography}, \emph{Orlando} draws from English takes real life as its source
material--except it re-works this material into what critics Christy
Burns and Jane de Gay describe as parody and satire.\footnote{Burns, Christy L. “Re-Dressing Feminist Identities: Tensions
between Essential and Constructed Selves in Virginia Woolf's Orlando.”
\textbf{Twentieth Century Literature}, vol. 40, no. 3, 1994, pp. 342–364.
\textbf{JSTOR}, www.jstor.org/stable/441560; De Gay, Jane. "Virginia Woolf's
...
Vita Sackville-West.\footnote{“‘If I Saw You Would You Kiss Me?": Sapphism and the
Subversiveness of Virginia Woolf's Orlando.” \emph{PMLA}, vol. 103, no. 1,
1988, pp. 24–34. www.jstor.org/stable/462459} Episodes from Sackville-West's real life,
...
pursue an \emph{iterative} text analysis method that runs the same
...
significant" (17).\footnote{Andrew Piper, \emph{Enumerations}, University of Chicago
Press, 2019. page 18.} Unlike Piper, however, who uses the
...
word vectors, and the \texttt{Word2Vec},\footnote{Link to Word2Vec source code:
\url{https://radimrehurek.com/gensim/models/word2vec.html\#gensim.models.word2vec.Word2Vec}} library in python, I compute
...
to "woman" and "man" in vector space.\footnote{The documentation describes \texttt{similar\_words()} "computes
...
while also being distant from "man" in vector space.\footnote{Link to file from this project's source code:
\url{https://github.com/gofilipa/qdr/blob/master/lib/actions.py}} I saved
these similar words in separate lists called \texttt{woman\_similar} and
\texttt{man\_similar}, as seen below.
...
genders are constituted by certain \emph{exclusions} which define the
...
stepping into "the yellow pools which \emph{chequered} the floor, made by
...
The image of Orlando's body cut or \emph{chequered} into different colored
...
wondered. 'For I must wire at once to Shel and tell him\ldots{}' And
...
life\ldots{} that's what it is--a toy boat on the Serpentine, ecstasy--it's
...
unstable, expressive qualities. Gender, like language, is \emph{fluid}. As
...
one sex to the other\ldots{} as well as through the shifting of her own
rhetoric in this novel" (42); Victoria L. Smith asserts that "what
happens in the novel\ldots{} and what it thematizes—-language’s inability
...
\label{sec:orgbd9fff8}

Like \emph{Orlando}'s narration, there is something conspicuous about my
own presentation of facts. Attentive readers of \emph{Orlando} will know
