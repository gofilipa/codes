...
\label{sec:org0e9ba57}

\subsubsection{polarization}
\label{sec:orga3f0491}
...
know then that we don't already know?" (\emph{Novel Gazing} 3-4). In
...
of it, the having and expressing of it?" (\emph{Novel Gazing} 4). This new
method, which she calls \emph{reparative reading}, embraces the productive
power of language, how it can coordinate seemingly unrelated
associations to generate new meaning.\footnote{Sedgwick fully develops what she "paranoid reading," also known
...
\emph{Touching Feeling: Affect, Pedagogy, Performativity}.}

Over two decades later, in the dawn of the age of automated text, this
project takes the prospect of \emph{generation} as a reading method quite
...
call \emph{aggregate forms} which surfaces sites of potential intersection,
and even solidarity, among polarized points of view.

I will demonstrate with an example, using a machine learning model
that I deliberately trained\footnote{To train this model, I used a "base" model, called gpt-2, which
...
"fine-tuning."} to represent a progressive point of
...
on articles about gender from the ACLU\footnote{The training data and source code used to scrape the articles
can be found on github.com/gofilipa/anti-trans under an open license.}, align with what one
...
Similarly, another model\footnote{Both of the models are openly licensed on Huggingface.co. See
Calado, \emph{gpt2-hertiage\_foundation-gender}, and Calado,
\emph{gpt2-aclu-gender}.}, which I trained on conservative media,
...
gender with \emph{identity}, which is internal rather than biological. The
...
conservative position; rather, it reflects a conservative \emph{frame} for
...
of embodiment from the popular heterosexual dating show, \emph{Love is
...
open licencses\footnote{To scrape the transcripts, I wrote a web crawler using the
...
(See Calado, \emph{love\_blind} code repository and Calado,
\emph{LoveIsBlind\_Pods} and \emph{LoveIsBlind\_Postpods} model repositories.)}). The resulting text generators synthesized
...
laptop, over a single afternoon.\footnote{For example, this project uses GPT-2, which is intially trained off
...
proprietary status.} Moreover, the dataset which I
...
\emph{self-reflexive}. As Wendy Chun points out, predictive tools are good
...
processes,\footnote{Include some of this eugenicist history of stats tools. Can be
brief.} the foundation for all machine learning technology
...
an apt tool for studying shared desires---in my case, with the \emph{LiB}
...
\subsubsection{sensory splits}
\label{sec:org005793f}
...
the "postpods model."\footnote{Link to the models!}
...
body appears to be re-integrated. The outer body comes into view when
the participants are finally given visual access to each other. Here,
the language about the body shifts into notably more visual and
...
what Prosser would call the "insentient visible body" (70). Because
the couples can see each other, the body feels "lighter" and "warm,"
offering coherence where before was weirdness and weight. In the last
output, however, there is a suggestion of something not quite right:
...
then, these subjects in \emph{LiB} develop romantic feelings and
attachments within the context of a sensory split, where their visual
sense is foreclosed from the other senses. And most of them, when they
leave the pods, cannot fulfill these aspirations within their embodied
lives.

\subsubsection{toward new solidarities}
\label{sec:orge536e89}
...
objectivity versus subjectivity, tradition versus liberation. Drawing
on Sedgwick’s reparative reading, I proposed text generation as a
method for working otherwise with polarized discourse—-not to resolve
...
prediction and approximation, can surface unexpected points of
overlap. What they reveal is not consensus, but intersection: shared
investments, shared anxieties, and shared attachments that persist
...
reflect a trans-affirming position so much as a conservative
caricature, which casts gender as internal and as a feeling. As Judith
Butler notes in \emph{Who’s Afraid of Gender?}, the contemporary far-right
fixation on biological sex is less a defense of scientific truth than
a reaction formation, which Butler calls a "phantasm," against the
perceived slipperiness of gender, its refusal to stay anchored to
stable referents. The insistence that sex is “binary and biological”
attempts to foreclose this instability by reasserting the body as a
fixed ground.
...
"Like language, gender categories\ldots{} are social and interpersonal, not
...
heterosexual dating on \emph{Love Is Blind}, this chapter sought to expand
...
desire, attachment, and disappointment.
...
which is the body, a contested yet shared site of becoming, for
everyone.

\subsection{Works Cited}
\label{sec:org328c1fe}
...
\emph{Representations} 1 May 2022; 158 (1): 106–119.
...
Economy. What Happens if It Falters?". \emph{The New York Times}. November
24, 2025.
...
\subsection{Appendix 1: ACLU and Heritage Model Outputs}
\label{sec:orgbd36195}
\subsection{Appendix 2: Pods Model Outputs}
\label{sec:org8a336c3}
...
\subsection{Appendix 3: Postpods Model Outputs}
\label{sec:org51e1383}
