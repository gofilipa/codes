...
\label{sec:orgeba0bfa}
\subsection{Polarization}
\label{sec:org840c50c}
...
we don't already know?" (\emph{Novel Gazing} 3-4). In Sedgwick's work, this
...
it?" (\emph{Novel Gazing} 4). This new method, which she calls \emph{reparative
reading}, embraces the productive power of language, how it can
coordinate seemingly unrelated associations to generate new
meaning.\footnote{Sedgwick fully develops what she "paranoid reading," also known
...
See Sedgwick, \emph{Touching Feeling: Affect, Pedagogy,
Performativity}.}
...
\emph{aggregate forms} which surfaces sites of potential intersection, and
even solidarity, among polarized points of view.

I will demonstrate with an example, using a machine learning model that
I deliberately trained\footnote{To train this model, I used a "base" model, called gpt-2, which
...
called "fine-tuning."} to represent a progressive point of view.
...
on articles about gender from the ACLU\footnote{The training data and source code used to scrape the articles can
be found on github.com/gofilipa/anti-trans under an open license.}, align with what one might
...
\emph{identity}, which is internal rather than biological. The American
...
embodiment from the popular heterosexual dating show, \emph{Love is Blind}. I
...
\label{sec:orgf23a2bb}
To examine this trajectory in \emph{LiB}, I created some text generation
...
open licences\footnote{To scrape the transcripts, I wrote a web crawler using the
...
\url{https://subslikescript.com/series/Love\_Is\_Blind-11704040}. The
...
versions remain free as well. (See Calado, \emph{love\_blind} code
repository and Calado, \emph{LoveIsBlind\_Pods} and
\emph{LoveIsBlind\_Postpods} model repositories.)}). The resulting text generators synthesized common
...
"bigger-is-better mentality" that drives ML development. This mentality
has to do with the belief that more data (scraped from the internet) and
more "compute" (Graphical Processing Units, or GPUs, sourced from deep
Earth minerals) will lead to better performing models. The drive for
larger models has spurred more and more investment, which has inflated
the economy to what some project are bubble-bursting levels, as many
tech companies like OpenAI are running on pure investment and do not
project to be actually profiting from their product for several years
(Casselman). Additionally, as recent research points out, this bigger is
better drive is counter-intuitive: Large Language Models actually have a
ceiling in terms of how size affects performance, that ever-increasing
compute does not yield comparable returns in terms of the quality of
model outputs (Varoquaux et al.). Which makes the tech companies all the
more desperate to protect their investments at all costs.
...
jokingly call "small language models," were trained on a single laptop,
over a single afternoon.\footnote{For example, this project uses GPT-2, which is initially trained
...
to its closed and proprietary status.} Moreover, the dataset which I used for
training was also small in size, containing the transcripts from just
...
apt tool for studying shared desires---in my case, with the \emph{LiB}
...
the transcript, like "marriage is not easy" and it blocks out the second
half of the sentence, so that only "marriage is" remains (\emph{Love Is
Blind}, Season 2, Episode 14). It tries to guess what should go in the
second half, perhaps guessing with the phrase, "Marriage is an apple."
Moving to the next step, loss, it checks its prediction against the
actual sentence, "Marriage is not easy." In this case, the concept of
"loss" represents the mathematical difference between the vector for
"not easy" and the vector for "apple." Then, it moves to the final step,
...
model."\footnote{Link to the models!}
...
altar. The reason, he later explains, is because he "struggled with\ldots{}
...
"get[ting]\ldots{} major ick." Though she claims her feeling of disgust have
...
touch you PODS
\begin{itemize}
\item I want to touch the couch.
\item I want to touch somebody.
\item I don't want to touch him, but I want to be his wife.
\item I don't want to touch him.
\item I don't want to touch anyone's feet.
\item I don't want to touch my feelings.
\end{itemize}

POSTPODS
\begin{itemize}
\item I want to touch you/him/her (repeated for all).
\item I don't want to touch you/him/her (repeated for all).
\end{itemize}
...
then, these subjects in \emph{LiB} develop romantic feelings and attachments
...
\begin{itemize}
\item The future with you, it's gonna be tenfold.
\item The future togetherness is gonna be great.
\item The future with her, she's gonna be there forever.
\end{itemize}

Postpods, the future is more negative:
\begin{itemize}
\item The future looks pretty bleak. (repeated)
\item The future has been absolutely horrible.
\end{itemize}
...
\begin{itemize}
\item Being intimate with you just makes me feel so safe.
\item Being intimate with you has made me feel so connected to you.
\item Being intimate with you has made me feel so alive.
\item Being intimate with you feels so sexy.
\item Being intimate is very sexy, but I don't wanna put on a lot of body
image.
\item Being intimate is fun.
\item Being intimate is good.
\item Being intimate is so sexy, and I've never felt so close, so deep, so
deep.
\item Being intimate is definitely very attractive, and it makes me feel
so safe and secure.
\item Being intimate, it's so easy to get to know someone and be
vulnerable.
\item Being intimate, it's very physical.
\item Being intimate has been very exciting.-
\item Intimacy is one of the biggest things I'm constantly insecure about.
\item Intimacy is definitely a fear.
\item Intimacy is not worth it for me.
\item Intimacy is not my thing.
\end{itemize}

POSTPODS
Intimacy described as desired but deferred:
\begin{itemize}
\item Being intimate with you is something that I've wanted for my entire
life.
\item Being intimate with you is something that I've never experienced.
\item Being intimate with you is something that I've wanted for my whole
life.
\item Being intimate with you is something that I could potentially do.
\item Being intimate with you is the most sacred thing.
\item Being intimate with you is really meaningful.
\item Being intimate with you is something that I need.
\item Being intimate with you is a big deal.
\item Being intimate with you is one of the most important things to me.
\item Being intimate with you is something that I've been looking for
since day one.
\item Being intimate with you is very rare for me.
\end{itemize}


\subsection{solidarities}
\label{sec:org26d0628}
...
the proof which will not, in Sedgwick's words, "\emph{intrinsically} or
\emph{necessarily} enjoin on that person any specific train of
epistemological or narrative consequences" (\emph{Novel Gazing} 4).

"To know that the origin of HIV \emph{realisitically might} have resultled
...
possible plot. They might, but then again, they might not" (\emph{Novel
Gazing} 4).
...
language, gender categories\ldots{} are social and interpersonal, not
...
end, he takes refuge in the sense of \emph{feeling}, in the untraceability
of felt sensation.
...
\emph{Who's Afraid of Gender?}, the contemporary far-right fixation on
