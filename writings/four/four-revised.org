...
demonstrate below, transforms language into what I call /aggregate
...
example. I trained a machine learning model[fn:2] specifically to
...
scraped from the ACLU[fn:3], align with what one might expect from a
...
Conversely, I trained another model[fn:4] to express a typically
...
dating show, /Love is Blind/. I chose this show, which features
...
this will not be the case for /LiB/'s cisgendered subjects, who are
...
by scraping them from a website.[fn:10] Then, Using the transcripts
...
released online under an open licence[fn:5]. The resulting text
...
the secretive nature of the training process.[fn:9] The dataset which
...
origins of statistical processes,[fn:7] the foundation for all machine
...
with the /LiB/ subjects, for studying a shared desire for marriage.

I will demonstrate an example how this normalizing mechanism works by
generating what I call /approximations/ of language patterns.
...
These approximations are a kind of /normalization/ of language. This
...
based on these training samples. The model /wants/ to find an average
...
readings of the cisgendered dating show, /Love Is Blind/.
...
model the "post-pods model."[fn:8]
...
words, the model is /too/ accurate. A model overfitting in its outputs
...
Screenshots from /LiB/ season 4, episode 4: "Playing with Fire".
...
from the material body... [and] can feel sufficiently substantial as
...
insentient visible body." (70). While the /LiB/ participants are
...
suffice]
- maybe consider adding /power to move/? Is that what we are missing
  from reading?
...
or /necessarily/ enjoin on that person any specific train of
...
[[./magnet.png]] Fig. Wikimedia Commons. Ironfilings cylindermagnet.svg
