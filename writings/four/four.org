...
*** introduction: reading practices, the body
**** Sedgwick's shift from knowledge to method

Over 25 years ago, in the midst of the first wave of the AIDS epidemic
and the government's neglect for those whom it affected (mostly Black
people and gay men), Eve Kosofsky Sedgwick wonders about the point of
doing critique in the first place. Even if critique could expose
governmental neglect of marginalized populations, that "the lives of
African Americans are worthless in the eyes of the United States; that
gay men and drug users are held cheap where they aren't actively
hated," Sedgwick wonders, "what would we know then that we don't
already know?" (/Novel Gazing/ 3-4).

In Sedgwick's work, this question indexes a shift her thinking about
reading practices, from a focus on knowledge, "Is a particular piece
of knowledge true?", toward method: "What does knowledge do---the
pursuit of it, the having and expressing of it?" (/Novel Gazing/ 4).
Sedgwick calls this new method /reparative reading/.[fn:9] This method
is about reading for the productive, generative effect of language,
for how language can coordinate seemingly unrelated associations or
effects.

Over two decades later, in the dawn of the age of automated text, this
project takes this prospect of /generation/ as a reading practice
quite literally, and applies it a very contemporary issue of gender.
While Sedgwick poses reparative reading as a hermeneutic that responds
to knowledge work and its impotence in the wake of the AIDS crisis, I
pose text generation, specifically by automated systems, as an apt
response to political polarization on the topic of gender and on trans
rights. This project uses generation as a tool for finding where
polarized perspectives surrounding gender might intersect. Machine
learning processes turn language into a form, which I call an
/aggregate form/ (explained below), which offers potential for
political solidarity and intersection, even among polarized points of
view.

**** Heritage/ACLU

First, I will demonstrate an example. I asked one text generator,
trained on "progressive" data, to describe gender terms, specifically
"masculinity" and "femininity". It generated the following outputs:
#+begin_quote
...
#+end_quote
These outputs align with what one might expect from a perspective that
affirms gender diversity and expression. This model, which I trained
myself on a custom dataset[fn:6] [fn:8]) based on articles about
gender from the ACLU,[fn:7] characterizes gender as a celebratory
phenomenon, using empowering language like "liberation," "beauty", and
"joy".

Meanwhile, in a similarly predictable way, another model associates
"Masculinity" and "Femininity" with what are typically conservative
ideals. This model, which I trained on articles from the Heritage
Foundation, generated the following outputs about gender:
#+begin_quote
...
#+end_quote
Like the progressive model, these gender terms are positive. However,
unlike the progressive model, the terms here affix to notions of
culture, tradition, and reproduction, which priotitize stability over
empowerment.

That being said, there is an outlier in these results, which centers
on a particular, term, "subjectivity." This term appears in ways that
one wouldn't typically associate with the conservative viewpoint:
#+begin_quote
...
#+end_quote
These examples do not describe the far-right viewpoint on
gender—--that it is based on the "biological truth" of two sexes, for
example, from executive orders like "Defending Women From Gender
Ideology Extremism And Restoring Biological Truth To The Federal
Government" and "Keeping Men Out of Women's Sports" (The White House).
Unlike these initiatives, which double down on the primacy of sex,
defining it as "binary and biological," these outputs more closely
resemble the progressive view of gender, which asserts that gender
describes identity, based on social behaviors, roles, and expression,
among other things (The White House 2025a, The White House 2025b). The
...
**** the ML process aggregates perspectives
In fact, the particular phrase, "gender is subjective", does not
reflect the conservative position from the Heritage Foundation data.
Rather, it reflects a conservative frame for the progressive position.
It represents what a conservative thinks a progressive person thinks
gender is---as something insubstantial, as a feeling. The outputs,
then, express not a single perspective of gender, but an /aggregation/
of perspectives into a single statement. It is the machine learning
process, which underlies the language model, that takes these distinct
...
This paper uses this aggregative method to surface commonality and
...
drives machine learning (ML) text generation processes, to trace how
this mechanism aggregates and "normalizes" language expressions, to
offer a politcally potent tool for combatting polarization 

I use the aggregative method on a dataset representing cisgendered
experiences of embodiment---specifically, from the popular
heterosexual dating show, /Love is Blind/. The show takes heterosexual
men and women, puts them into a "blind" dating experiment with the
ultimate goal of marriage. First, the daters are placed into dating
"pods" where they can hear but not see the other. Then, once they
agree to get married, they meet and live together for one month until
their wedding day, making their final decision, whether or not to
remain together, at the altar.

*** /LiB/'s transgressive premise
Similarly to the two models that I trained on conservative and
progressive perspectives, I trained two models based on the
transcripts of the show: the first model is trained on the transcripts
from the episodes in the pods, when the participants fall in love and
agree to get married without ever seeing the other; and the second
model, I trained off the period following the pods, when the daters
finally meet and live together, which is when the relationships tend
to fall apart.[fn:10]

I partitioned the models in this way deliberately so that I could
study how the body, and how the introduction of the body, affects the
dating experiment. While the point of going on the show, as one
participant puts it, is "to be loved for who I am on the inside,"
something very differnt is happening here (Season 2, Episode 3, "Love
in Paradise”). The show’s main gambit, that "love is blind,” suggests
a transgressive premise that undergirds an ultimately heteronormative
teleology. Despite the promise of marriage and happily-ever-after, the
experiment poses embodiment in a non-normative function. It explores
the body, and what happens to the body when it falls in love from
behind a wall.

The main readings of this paper explore bodily dissonance in the
experiences of these cisgender, heterosexual daters. I take
theorizations of embodiment from Trans Studies and apply them to an
analysis of these cisgendered subjects. I examine what this situation,
...
the internet,[fn:1] I built a text generator that mimics the speech of
...
*** TODO ADD citations to recent reporting on AI
...
"bigger-is-better mentality" that drives ML development.[fn:2] This
...
single afternoon.[fn:3] The dataset which I used for training (more on this
process below) was similarly small in size, containing the transcripts
from one season, 13 total episodes, of the show.[fn:4]
...
learning as a tool for productivity, to generate /new/ content, while
serving extractive and monetizing purposes, misses the fact that these
tools are primarily /self-reflexive/. As Wendy Chun points out,
...
eugenicist, origins of statistical processes,[fn:5] which forms the
...
and amplify /frequent patterns/ of word usage. This mechanism of
...
studying shared desires---in my case, with the /LiB/ subjects, for
...
seminal work /Second Skins: The Body Narratives of Transsexuality/
(1998), explains that trans stories seek embodied normativity as a
/telos/. Prosser's argument, which I examine in more depth below,
...
to be nonperformative, to be constative, quite simply, /to be/" (32).
...
body split creates "conflict... between sentient body image and
...
https://huggingface.co/gofilipa/LoveIsBlind_Postpods.
2025.
...
[fn:9] Sedgwick here critiques what she calls "paranoid reading," also
known as "suspicious reading" or the "hermeneutics of suspicion," an
idea that she fully develops in her famous essay, "Paranoid Reading
and Reparative Reading, or, You’re So Paranoid, You Probably Think
This Essay Is About You." See Sedgwick, /Touching Feeling: Affect,
Pedagogy, Performativity/.

[fn:8] For those who don't know, training models happens in various
...
training the underlying base model.

[fn:7] The training data and source code used to scrape the articles
can be found on github.com/gofilipa/anti-trans.
...
my github account, under the "gofilipa/love_blind repository," (see
Calado, "love_blind".

[fn:2] I do not use the term "AI," but machine learning.
...
[fn:4] The code, training data, and models developed for this project
...
. published under https://github.com/gofilipa/love_blind and
the models are published under https://huggingface.co/gofilipa
