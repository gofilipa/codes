...
\label{sec:org9a4cbea}

\subsection{draft}
\label{sec:orgaaec0ec}

\subsubsection{introduction}
\label{sec:orgedb224f}
...
\label{sec:orgea5169a}
\subsubsection{ACLU close reading}
\label{sec:org1315756}
...
\subsubsection{"comments" quotes on original/copying}
\label{sec:org23b8537}
...
\subsubsection{aclu quotes}
\label{sec:org368402d}
...
\subsubsection{heritage quotes}
\label{sec:orgbd0f211}
...
\begin{enumerate}
\item commericalization
\label{sec:orgecc2301}
...
\item fair use, campbell case
\label{sec:orgf4ba933}
\begin{quote}
...
\end{enumerate}
\subsubsection{move to draft}
\label{sec:orgea5d1f4}
...
\label{sec:org8bfa942}
\subsubsection{Chandrasekhar 2025}
\label{sec:org33ca49b}
\begin{itemize}
\item how do copyleft licenses transfer to datasets, models, tokens?
\item EleutherAI developing the Pile V2
\item Problem isn't that data is used without compensation, but that
products/outcomes are not contributed back to the commons (19).
\item are parameter's "transformative"?
\item The issue becomes: who has the ability to create? To use the GPUs.
\item alternative licenses:
\begin{itemize}
\item Nwulite Odobo "dual regime" - free for users in developing
countries, multiple licensors for a dataset
\item Kaitiakitanga - royalties go to community, community ownership
\end{itemize}
\item language is extractive, indigenous communities know this.
\end{itemize}
\subsubsection{The Author’s Guild v. Hathitrust, 2014}
\label{sec:org918681a}
...
\subsubsection{Authors Guild v. Google, Inc., No. 13-4829 (2d Cir. 2015)}
\label{sec:org3487279}
...
\subsubsection{“Winning the AI Race: Strengthening US Capabilities in Computing and Innovation.Sam Altman, Testimony, May 8:}
\label{sec:org9ebbe8d}
\begin{itemize}
\item May 8 congressional hearing titled “Winning the AI Race:
Strengthening US Capabilities in Computing and Innovation.”
\item OpenAI CEO Sam Altman, Microsoft President Brad Smith, AMD CEO Dr.
Lisa Su, and CoreWeave CEO Michael Intrator speaking to the Senate
Commerce Committee.
\item Argument: that the US requires free rein (low regulation) to defeat
China in the "AI Race", we will know we win the race if we can
innovate and export"
\item Cruz:
\begin{itemize}
\item position: regulation is "needless" and "orwellian",
"paternalistic".
\item Cruz's contradictory language frames US as free, Europe and China
as authoritarian.
\begin{itemize}
\item Cruz's language contrasts "entrepreneurial freedom and
technological innovation" against "command-and-control policies
of Europe".
\end{itemize}
\item Drawing from history of the internet, which was developed with
relatively low regulation in the USA.
\begin{itemize}
\item Telecommunications Act of 1996 that promoted competition via
...
\end{itemize}
\item Referring to Biden and some state legislatures: "They want a
testing regime\ldots{} seemingly something out of Orwell \ldots{} as if AI
engineers lack the intelligence to responsibly build AI without
the bureaucrats"
\item "U.S. dominance in AI depends on two factors: innovation and
adoption."
\end{itemize}
\item Altman:
\begin{itemize}
\item vetting systems would be "disastrous" for industry, "sensible
regulation that does not slow us down"
\end{itemize}
\item Smith, microsoft president:
\begin{itemize}
\item the way to know we've won the "race" is if our tech is broadly
adopted.
\end{itemize}
\end{itemize}

\subsubsection{NYTimes complaint}
\label{sec:orgcd01747}
\begin{itemize}
\item NYT complaint argues that OpenAI "stea[s] audiences away from it",
...
\subsubsection{2018 OpenAI press release, december 12 2018, "Introducing OpenAI"}
\label{sec:orgb4653ea}
\begin{itemize}
\item OpenAI started as a nonprofit, and raised money with promises to
share their products freely:
\begin{itemize}
\item "Researchers will be strongly encouraged to publish their work,
whether as papers, blog posts, or code, and our patents⁠ (if any)
will be shared with the world" (OpenAI 12/11/2018 press release).
\end{itemize}
\end{itemize}

\subsubsection{2025 "OpenAI’s proposals for the U.S. AI Action Plan" march 13, 2025}
\label{sec:org9ccc630}
\begin{itemize}
\item "we must ensure that people have freedom of intelligence, by which
we mean the freedom to access and benefit from AI as it advances"
\item "freedom-focused policy proposals"
\item "neutralizes potential PRC benefit from American AI companies having
to comply with overly burdensome state laws."
\begin{itemize}
\item "freedom to innovate" regulations
\item "copyright strategy that promotes the freedom to learn"
\begin{itemize}
\item "secure Americans’ freedom to learn from AI"
\item "avoid forfeiting our AI lead to the PRC"
\end{itemize}
\item "export strategy"
\item develop infrastructure
\item adoption by government
\end{itemize}
\end{itemize}

\subsubsection{2025 OSTP OSTP proposal, march 13, 2025}
\label{sec:org337ed33}
\begin{itemize}
\item Office of Science and Technology Policy proposal
\item proposals to help OSTP develop "AI Action Plan \ldots{} that ensure[s]
that American-led AI built on democratic principles continues to
prevail over CCP-build autocratic, authoritarian AI".
\item "democratic AI"
\begin{itemize}
\item "a free market promoting free and fair competition.
\item "freedom for developers and users to work with our tools"
\item "preventing government use\ldots{} to amass power and control their
citizens"
\end{itemize}
\item Deepseek is a threat because "simultaneously state-subsidized,
state-controlled, and fully available\ldots{} cost[ing] users privacy and
security."
\item point \#3: "Copyright: Promoting the Freedom to Learn"
\begin{itemize}
\item need to use copyrighted material to compete with China, a "matter
of national security."
\item contradiction between China's "unfettered access" vs OpenAI's
"fair use":
\begin{itemize}
\item "Applying the fair use doctrine to AI is not only a matter of
...
\end{itemize}
\end{itemize}
\subsubsection{2023(?) OpenAI Comments on Intellectual Property Protection for Artificial Intelligence Innovation}
\label{sec:org4f33706}
\begin{itemize}
\item argue that, “Under current law, training AI systems constitutes fair
use”
\begin{itemize}
\item argument for fair use hinges on "transformative" use of copyrighted work
\begin{itemize}
\item citing a passage from a court case that defends parody (Campbell
v. Acuff-Rose Music) as fair use to argue that AI outputs are
"highly transformative"
\item input data: copyrighted works become statistical patterns,
“non-expressive”"
\item output data: nobody can use AI to read the specific webpages
they are trained on: they will still go to NYTimes to read the
news. (debatable).
\end{itemize}
\end{itemize}
\item "mission is to ensure that artificial general intelligence (“AGI”)
benefits all of humanity”"
\item anthropomorphize AI training into human learning:
\begin{itemize}
\item “does copyright law’s protection of an author’s original
expression impede AI systems from generating insights about that
expression?”
\item ““training” refers to the process by which an AI model learns
patterns”
\end{itemize}
\item “Authors may object that the outputs of generative AI systems will
harm the value of their works. We address this objection in Section
II.”
\begin{itemize}
\item “Distributive Issues from AI-Generated Non-Infringing Works Should
Be Addressed by Other Policies”
\item “this concern falls into a broader category of concerns about the
relationship  between automation, labor, and economic growth”
\item "we believe that such distributive claims are most efficiently
addressed through taxation and redistribution, rather than
copyright policy.”
\begin{itemize}
\item “Louis Kaplow \& Steven Shavell, Why the Legal System Is Less
